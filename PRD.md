...
Features shipped in v1.1, based on findings from user research sessions conducted in April 2026.

### 5b.1 Query Progress Indicator

While the agent is processing a query, display real-time status messages showing each step of the process. This gives users visibility into what the agent is doing and increases trust in the results.
...
**Implementation decisions (shipped):**
- Uses Streamlit's `st.status()` widget with three steps: Generating KPIs → Verifying confidence scores → Saving results
- Status expands automatically during the query and collapses with a completion message when done

### 5b.2 Granular Confidence Scoring

Supplement the three-tier confidence signal (high / medium / low) with a finer-grained numeric score to help users better differentiate KPI reliability.

- **Source:** Session 2 — PM users found the current three tiers insufficient

**Implementation decisions (shipped):**
- Added `confidence_score` (1–10) alongside the existing high/medium/low label
- Score generated by Claude and updated by the web verification step
- Displayed on each KPI card as e.g. "🟢 High · 9/10"
- KPIs sorted by `confidence_score` descending; old records without a score fall to the bottom

### 5b.3 Results Summary

Display a brief narrative summary at the top of the KPI results page providing industry context before the KPI list.

- **Source:** Session 2 — PM users requested more context alongside the KPI list

**Implementation decisions (shipped):**
- `summary` field added to the system prompt — Claude returns a 2–3 sentence industry overview in the same API call
- Displayed between the query timestamp and the KPI cards

### 5b.4 Query History View

Surface past queries in the UI so users can return to prior results without re-running a search.
